...
class NoteSyncControllerTest extends AppTest {
    ...
    static void beforeAllTests() {
        ...
        final module = new JavaTimeModule()
        module.addDeserializer(Instant, new StdDeserializer<Instant>(Instant) {
            @Override
            Instant deserialize(final JsonParser parser, final DeserializationContext context) {
                final time = parser.getValueAsString()
                return Instant.parse(time)
            }
        })
        module.addSerializer(new StdSerializer<Instant>(Instant) {
            @Override
            void serialize(final Instant time, final JsonGenerator generator, final SerializerProvider provider) {
                final formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'").withZone(ZoneOffset.UTC)
    ...
    @Test
    void shouldSyncNotes() {
        final externalNotes = new Note[]{
            new Note(
        ...
        final noteGraves = new NoteGrave[]{
        ...
        final requestBody = createRequestBody(externalNotes)
        createNotes(notes)
        ...
        final responseTree = objectMapper.readTree(response.body())
        assertThat(responseTree.size()).isEqualTo(1)
        final notesResponse = objectMapper.readValue(
        ...
        final noteGravesDb = readNoteGraves()
    ...
    private static void createNote(final Note note) {
        dao.executeUpdate(
    ...
    private static void createNotes(final Note... notes) {
        for (final note : notes) {
            createNote(note)
        }
    }
    ...
    private static void createNoteGrave(final NoteGrave noteGrave) {
        dao.executeUpdate(
    ...
    private static void createNoteGraves(final NoteGrave... noteGraves) {
        for (final noteGrave : noteGraves) {
            createNoteGrave(noteGrave)
        }
    }

    private static List<NoteGrave> readNoteGraves() {
        return dao.executeQuery('SELECT * FROM note_grave', {
            new NoteGrave(
                UUID.fromString(it.getString('id')),
                it.getTimestamp('creation_time').toLocalDateTime().toInstant(ZoneOffset.UTC)
            )
    ...
    private static String createRequestBody(final Note[] notes) {
        final noteRequests = Arrays.asList(notes).stream()
